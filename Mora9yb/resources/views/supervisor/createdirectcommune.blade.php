@extends('supervisor.communeform')
   
                     
                    @section('contentcommuneDrctDropdown')

                     <form class="login100-form validate-form" method="POST" action="/communetask">
                            @csrf 
                            {{-- @method('PUT') --}}

                            {{-- <livewire:communedrct/> --}}
                            @livewire('communedrct', ['selectedCentre' => 1])
                            {{-- @livewire('commundrct') --}}
                    @endsection
                    @section('content2')
